...
- **控制台只列 Linux 包（2026-09-22）**：手装 ai-bridge 的都是机房服务器；macOS / Windows 上
  接进来的是 Nova 客户端，它自带一份。平台照旧可以发这两种系统的包 —— 分发面、`install.ps1`、
  远程升级都没动 —— 只是 Nova 账户页的「安装 ai-bridge」不列它们，一行命令也只给 sh 那条。
...
### 数据怎么来：问本机的 claude / codex 自己

不猜上游的 HTTP 端点，直接问已经知道答案的那个程序 —— 屏幕上那两个数本来就是它们显示的：

| 上游 | 怎么问 | 拿到什么 |
|---|---|---|
| Claude | `claude -p "/usage" --output-format json` | 正文里的 `Current session: 17% used · resets …` 几行，解文本 |
| Codex | `codex app-server` 的 stdio JSON-RPC，方法 `account/rateLimits/read` | **结构化 JSON**：`primary` / `secondary` 各带 `usedPercent`、`resetsAt`、`windowDurationMins` |

两条都**不花 token**：Claude 那条是客户端侧的斜杠命令（实测 0 input / 0 output），
Codex 那条是一次用量读取，都不触发模型推理。也都**不碰凭据** —— 两个 CLI 读的是
它们自己的登录态，令牌不经过 bridge。

窗口是**自描述**的，不用猜谁是 5 小时谁是一周：Codex 给 `windowDurationMins`
（300 → `5h`，10080 → `7d`），Claude 的「Current session」是 5 小时窗口、
「Current week」是周窗口。Claude 会加新窗口（`Current week (Fable)` 就是后加的），
所以解析按「`标题: N% used`」的形状认，不写死标题文字。

**统一存「已用」百分比。** Claude 报的是已用（`17% used`），Codex 的 `/status`
屏幕上显示的是剩余（`63% left`）—— 两边各存各的说法，就一定有某一处会转反，
而转反之后数字看着仍然合理，没人会发现。换算只在界面做一次。

节点每 **5 分钟**探一次，而不是跟着心跳（15 秒）：这两条都要起子进程，
而这个数在五分钟里变不出花来。只探**这台机器真的在共享的那些上游** ——
只共享 codex 的机器不该被起一个 claude 进程。

探不到（CLI 没装、没登录、超时、输出格式变了）时**保留上一次的观测**，
不写一份空的进去：界面从「剩 63%」跳成「未报」，和真的没数据是两回事。

CLI 路径默认取 PATH 里的 `claude` / `codex`，可以用 `AI_BRIDGE_CLAUDE_BIN` /
`AI_BRIDGE_CODEX_BIN` 覆盖 —— bridge 常被当守护进程跑（LaunchAgent、systemd），
那种环境的 PATH 和主人登录时的不是一回事。

### 试过但撤掉的：从中转响应的限流头里捎

最早做的是这个：每条中转请求的响应头里挑出 `*ratelimit*` 收下来。它不打上游、
免费、实时，但**给不出你要的那两个数**，三条限制每一条都是死的：

- **机器闲着就不更新** —— 而「此刻还剩多少」恰恰是闲着时最想知道的；
- **只在别人派活进来时才触发** —— 主人自己用 Claude Code 是直连的，不经过 bridge，
  所以他自己账号的余额这条路永远看不到；
- **只能拿到上游恰好放在响应头里的东西** —— 5 小时和周额度并不在那儿。

所以整条撤掉了，没留成第二个数据来源：两个来源喂同一个字段，界面会在两种形状
之间跳。撤掉时还带出两个自己埋的坑，都值得记一下 ——
喂给采集器的是**已经过滤过**的响应头（白名单只留 `anthropic-ratelimit-*` /
`x-ratelimit-*`，Codex 换个命名就一个都收不到）；凭据黑名单按子串挡 `token`，
把 `x-ratelimit-limit-tokens` 这类头**全挡掉了**，而 tokens 正是最该收的计量单位。
两个的表现都是界面上静默的一片「未报」，没有任何一条错误。

### 它进一个判定：余量下限（2026-09-21）

原先这一节写的是「它不进任何判定」，理由是上游的形状不稳定、节点自报的数没法验证。
两条理由今天仍然成立，但中间变了一件事：**数据来源换了**。从中转响应头里捎的那份
「只在跑活时才更新」，拿它当闸门会死锁 —— 低于线就不接单、不接单就没有请求、
没有请求数字就永远不刷新。换成定时探测之后，这个环断了：窗口重置后的下一轮探测
（≤5 分钟）就把数带回来，闸门自己打开。

于是主人可以在共享设置里划一条线：**上游某个窗口剩到这儿，这条贡献就不再接新单**，
剩下的留给自己用。规则存在 `zt_galaxy_contribution.upstream_floor_json`：

```json
[{"window": "", "percent": 0}, {"window": "7d", "percent": 20}]
```

`window` 为空是「任一窗口」（取各窗口里最低的那个）；`percent` 是**剩余**百分比的
下限，剩余 ≤ 它就停。必填，默认 `[{"window":"","percent":0}]` —— 剩 0 才停，
也就是和加这条线之前一样。一个窗口只能有一条线：两条盯同一个窗口时宽的那条永远
不生效，而主人看着界面上明明写着的数字，不会想到它是死的。

判定是**单向**的，那两条老理由留下的就是这个（`service/galaxy/upstreamfloor.go`）：
...
也就是这条线只会让一台机器少接活，不会让它多接活。反过来把「不知道」当成「没余量」
的话，上游改一次输出格式就能让全网的机器一起静默退出池子。

判定要落在**每一条**放置路径上，不只是硬过滤 —— 三条都问同一句话（`UpstreamHolding`）：

| 路径 | 占比 | 到线时 |
|---|---|---|
| 硬过滤 + 打分 | 生客 | 不进候选；`Waitable` 也认它，所以是立刻 503 而不是干等 maxWait |
| 上次落在哪台（绑定） | 约九成 | 退出快路径并**解绑**，这一单去挑别的机器 |
| 硬钉（`previous_response_id`） | 链式会话 | 和原节点离线同样处置：`node_unavailable`，不改派 |

只在硬过滤里判的话，这条线就只拦得住生客：老客户顺着绑定那条路照样一单一单地
把主人留给自己的那部分跑光，而共享设置页上一切正常。链式那条同理，一个聊得正欢的
会话能顺着它一路跑下去。

派单那一侧读的是控制面里的两个字段，不回头查库（放置是按 QPS 走的路径）：
规则 `upstreamFloor` 跟着 hello / 改设置写，观测 `upstreamLeft` 跟着心跳写（15 秒）。
节点这一拍没带余量时**不写**，留着上一次的观测 —— 空的盖上去等于让闸门在探针
抽风的那几分钟里凭空打开。

界面上它是**第三种状态**：主人没关（`status` 还是 active）、机器也好着
（`available` 为真），只是上游快用完了。贡献视图上的 `upstreamBlock` 会说清是哪个桶、
剩多少、线在哪，三个数一起给 —— 只说一句「暂不接单」的话，主人分不清该去改设置
还是该等窗口重置。它不改 `status`，也不算排空：额度触顶那条路是 `draining`，
两件事要分得开。
...
- **要那台机器上装着并登录着对应的 CLI。** 对共享者来说这基本成立（没装 Claude Code
  就没有 Claude 订阅可共享），但 API-key 接入的节点没有「订阅余量」这回事，不探。
- **Claude 那条要解文本。** `--output-format json` 只是把人类可读的那段塞进 `result`，
  数字仍然在文本里。Claude Code 改一次输出格式解析就得跟着改 —— 解不动时会记一条
  `usage_probe_claude_unparsed` 并把原文带上，照着改就行。
