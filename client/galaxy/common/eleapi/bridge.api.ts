...
import type { BridgePing, BridgePingOptions, BridgeState, BridgeHubSync, BridgePairPayload, BridgePairResult, UpstreamLoginResult, ToolStatus, ToolJob, BridgeRuntimeStatus, BridgeToken, BridgeTokenInput, BridgeIssuedToken } from './bridge.model';
...
export class BridgeApi extends ElectronApi {
  ...
  /** 拉起一次安装 / 升级，立刻返回那条进度记录；跑到哪了看 getTools 里的 job。 */
  @Invoke upgradeTool(tool: string): Promise<{ command: string; job: ToolJob }> { return this.invokeApi('upgradeTool', tool); }
