...
# 在**服务器**上解包 orbit-webview 的发布包。把 tar.gz 和这个脚本放同一个目录，
...
usage() {
  ...
  echo "Example: $0 orbit-webview-linux-x64.tar.gz $SCRIPT_DIR" >&2
}
...
if [ -z "$ARCHIVE" ]; then
  ARCHIVE="$(find "$SCRIPT_DIR" -maxdepth 1 -type f -name 'orbit-webview-*.tar.gz' | sort | sed -n '1p')"
  if [ -z "$ARCHIVE" ]; then
    usage
    exit 1
  fi
