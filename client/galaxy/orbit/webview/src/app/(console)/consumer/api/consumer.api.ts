...
export class ConsumerKeyView {
  ...
  /**
   * 这把密钥能用的模型档次，服务端已经连同模型名、档次名解好。
   *
   * 空 = 老密钥（2026-09-22 之前签的）：每个模型走它的默认档，照旧能用。
   * 新建的密钥一律要选档次 —— 中转按档次走，价钱和强度都由它定。
   */
  groups: KeyGroupView[] = [];
 ...
 */
/**
 * 新建一把密钥。**档次必选**：中转按档次走 —— 选了哪一档，这把密钥就按那一档的
 * 价钱、那一档卖的思考深度来跑。一个模型只能选一档。
 */
...
/**
 * 一个可选的档次：哪个模型、哪一档、什么价。
 *
 * 带着价给是有意的：选档次就是选价钱，把价留在模型广场那一页，
 * 人得开两个标签页对着看。
 */
...
export class UsageRecord {
  ...
  /** 这一次落在哪一档上。单价按 (模型, 档次) 定，「为什么扣这么多」要靠它才答得完整。 */
  groupId = "";
...
/**
 * 一个模型底下的一个**档次**：平台在这个模型上卖的一档，以及它的价。
 *
 * 卡片上那几个数是**模型通价** —— 没单独定价的档都按它收。这个列表列的是
 * 这个模型在卖的档次，一个都没有时是空的（那种模型建不出密钥来）。
 *
 * 档次取代了原先按「推理强度」分档的那张表（2026-09-22）：强度档位是上游的内部刻度，
 * 平台卖的是说得清楚的档次。服务端仍叫 group，界面上一律叫档次。
 */
