...
# 启动门户站。跑的是 .desktop/portal 里那份 Next standalone —— 也就是真正要
# 部署上服务器的东西，本机起它是为了「验的和发的是同一份」。
#
# 同一个文件在两处都能用：解包后的发布目录里，它和 standalone 并排；
# 源码树里，产物在 ../.desktop/portal。下面先认自己身边的，再回落到源码树。
#
# 写成 POSIX sh 而不是 bash：部署机上顺手一个 `sh start.sh` 就会掉进 dash，
# bash 那套 [[ ]]、set -o pipefail 在那儿会直接报 Illegal option。
APP_NAME="portal"
# 和 portal/package.json 里的端口一致；也是 nginx 里 galaxy_portal 指着的那个。
DEFAULT_PORT=17900

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BUNDLE_DIR="$SCRIPT_DIR"
if [ ! -f "$BUNDLE_DIR/portal/server.js" ]; then
  BUNDLE_DIR="$SCRIPT_DIR/../.desktop/portal"
fi
if [ ! -f "$BUNDLE_DIR/portal/server.js" ]; then
...
#
# 门户认的是：
#   SERVER_TARGET        业务代理打哪台 Galaxy API（只打 /api/galaxy/portal/*，
#                        那组在 galaxy-consumer-api，默认 10005）
#   APP_URL_PREFIX       代理前缀，默认 /api
#   GALAXY_CONSOLE_URL   「控制台」按钮指向哪儿（使用端 Orbit 的地址）
#   GALAXY_DOCS_URL / GALAXY_CONTACT_EMAIL / GALAXY_CONTACT_WECHAT / GALAXY_ICP
#                        页脚与「联系我们」上的链接与联系方式
#
# 这几个值是页面**每次渲染时现读**的（src/utils/site.server.ts），改完重启就生效，
# 不用重新构建 —— 早先它们叫 NEXT_PUBLIC_*，那种写法是构建期内联，
# 线上出过「控制台按钮指着打包机的 127.0.0.1」。旧名字仍然认，但请改成新的。
...
if [ -z "${SERVER_TARGET:-}" ]; then
  echo "没有 SERVER_TARGET：页面能开，模型与定价会取不到数。写进 $RUNTIME_FILE 或用环境变量给。" >&2
fi
if [ -z "${GALAXY_CONSOLE_URL:-}" ] && [ -z "${NEXT_PUBLIC_CONSOLE_URL:-}" ]; then
  echo "没有 GALAXY_CONSOLE_URL：「控制台」按钮会落到 /orbit/consumer/keys（同域部署时正好对，" >&2
  echo "分域部署会点到门户自己的 404）。写进 $RUNTIME_FILE。" >&2
fi
...
# 探的是 /api/health：它只回「门户自己活着」，不打后端 ——
# 后端没起的时候也应该能判断进程起没起来。门户挂在站点根上，不像两个端有 basePath。
i=0
while [ "$i" -lt 30 ]; do
  if curl -fsS "http://$PROBE_HOST:$PORT/api/health" 2>/dev/null | grep -q '"portal"'; then
    echo "$APP_NAME started on $LISTEN_HOST:$PORT (pid $(cat "$PID_FILE"))"
    exit 0
  fi
  i=$((i + 1))
  sleep 1
