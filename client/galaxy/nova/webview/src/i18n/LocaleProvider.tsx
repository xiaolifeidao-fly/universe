...
const messages = {
  "zh-CN": {
    ...
    "account.unbindHint": "解绑后这台机器不再接单，未结算的积分照常在争议期后到账。重新接入要再配对或重新注册一次。",
    ...
    "account.keyLastUsed": "最近注册 {time} · {node}",
    ...
    "account.cliHint": "注册后接入密钥会保存在那台机器的配置里，重启时自动重新注册。两种接入方式都能在上方的机器列表里看到状态。",
  ...
  "en-US": {
    ...
    "account.unbindHint": "After unpairing this machine takes no more work. Unsettled credits still land after the dispute window. Reconnecting means pairing or registering again.",
    ...
    "account.keysNone": "No access keys issued yet",
    ...
    "account.keyLastUsed": "last registered {time} · {node}",
    ...
    "account.cliHint": "The key is saved in that machine's config, so it re-registers on every restart. Both modes show their status in the machine list above.",
