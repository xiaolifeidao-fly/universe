 ...
 *
 * 只给 Linux：要单独装 ai-bridge 的都是机房服务器，不是主人坐着的这台 —— 这台的 Nova 自带一份。
 * 平台上发过的 macOS / Windows 包这一块不列，也不给它们的命令：这两种系统上要接进来的是 Nova，
 * 不是手装的 ai-bridge。
 *
...
import { Blank, CommandBlock } from "./parts";

/** 只认这个系统的包，平台名里是 <系统>-<架构> 的前半段。 */
const OS = "linux";
...
function accessArgs(access: Access, publicUrl: string, port: string): string[] {
  if (access !== "export") return [];
  const args = ["--mode", "export", "--public-url", publicUrl];
  const listen = port.trim();
  // 端口就是默认值时不写出来：配置里本来就是它，多一个参数只是多一处要对的地方。
  if (listen && listen !== DEFAULT_PORT) args.push("--port", listen);
  return args;
}
 ...
 */
function manualCommand(item: BridgeReleasePackage, hub: string, key: string, extra: string[]): string {
  const file = item.fileName ?? "";
  // 这几行调的是 ai-bridge 可执行文件本身，不是安装脚本。
...
export function BridgeInstallPanel({
  ...
  // 只留 Linux 的包，平台上别的系统发过什么这里都不认（见文件头注释）。
  const mine = (manifest?.platforms ?? []).filter((item) => splitPlatform(item.platform).os === OS);
  const hub = manifest?.hubUrl || hubUrl || "https://hub.example.com";
  const script = manifest?.installScript ?? "";
  const publicUrl = publicUrlOf(address, port);
  const accessKey = key.trim() || KEY_PLACEHOLDER;
  // 一行命令走的是安装脚本，手动那几行走的是 ai-bridge 自己，多带的参数两边同一套写法。
  const args = accessArgs(access, publicUrl, port);
  const oneLiner = script ? unixInstallCommand(script, accessKey, args) : "";
  ...
  if (!manifest) {
    ...
    // 平台一个包都没发、或只发了别的系统，对这一块都是同一件事：没有能装的东西。
    body = <Blank title={t("install.empty")} hint={t("install.emptyHint")} />;
  } else {
    body = (
      <>
        {/* 标题下这道线原来是系统页签带的，页签撤了就挂在这一块上，卡片的节奏不变。 */}
        <div
          style={{ display: "flex", flexDirection: "column", gap: 12, padding: "14px 18px 12px", borderTop: "1px solid var(--gx-line)" }}
        >
          ...
          {showManual ? (
            <>
              <CommandBlock
                ...
                command={manualCommand(mine[0], hub, accessKey, args)}
              />
              <Note>{t("install.manualNote")}</Note>
            </>
          ) : null}
        </div>
      </>
