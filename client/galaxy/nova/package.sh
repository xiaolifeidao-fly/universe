...
# Nova（共享端）桌面客户端的打包：只出**安装包**，落在 release/nova/。
#
# 不顺带构建界面那份 standalone —— 安装包里没有 Next 服务，壳只负责 loadURL
# 远端地址，界面是另一条发版线（nova/webview/package.sh 打 tar.gz 上服务器）。
# 少跑这一步还避开一件事：界面的构建会重写 nova/webview/.next，本机常驻着
# next dev 的话那个开发服务器会当场开始报 ChunkLoadError。
# 两样都要就在工作区跑 ../package.sh nova（等价于 npm run package:nova）。
PRODUCT="nova"
