...
import { Coins, LoaderCircle, RotateCw } from "lucide-react";
    ...
    <Sheet
      ...
      actions={
        <button className="icon-button" type="button" onClick={() => void load()} aria-label="刷新" disabled={loading}>
          {loading ? <LoaderCircle size={18} className="spin-icon" /> : <RotateCw size={18} />}
        </button>
      }
    >
              ...
              <div className="usage-rows">
                {usage.conversationGroups.map((group) => (
                  <div className="usage-row" key={group.key}>
                    <span className="usage-row__name">{groupLabels[group.key] ?? group.key}</span>
    ...
    <div className="usage-rows">
      {(Object.keys(providerLabels) as (keyof typeof providerLabels)[]).map((provider) => (
        <div className="usage-row" key={provider}>
          <span className="usage-row__name"><Coins size={14} aria-hidden="true" />{providerLabels[provider]}</span>
