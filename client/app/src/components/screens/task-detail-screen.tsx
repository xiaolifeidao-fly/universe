...
export function TaskDetailScreen() {
  ...
  return (
    <main className="screen">
      <div className="screen-title-row"><div><p className="eyebrow">任务详情</p><h1>{item.title}</h1><p>{phaseLabel(item.phase)}阶段 · 版本 {item.version}</p></div><Link className="icon-button" href={`/projects/${programId}`} aria-label="返回项目" title="返回项目"><ArrowLeft size={20} /></Link></div>
