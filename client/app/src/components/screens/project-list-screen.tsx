...
import { ArrowRight, AlertTriangle, FolderKanban, RefreshCw } from "lucide-react";
  ...
  return (
    <main className="screen">
      <div className="screen-title-row">
        ...
        <button className="icon-button" type="button" onClick={() => void load()} aria-label="刷新项目" title="刷新项目" disabled={loading}><RefreshCw size={20} className={loading ? "spin-icon" : ""} /></button>
      </div>
      {loading ? <LoadingState title="正在读取项目" /> : null}
      {!loading && error ? <EmptyState icon={<AlertTriangle size={21} />} title="暂时无法读取项目" description={error} action={<button className="button button-primary" type="button" onClick={() => void load()}>重新连接</button>} /> : null}
      {!loading && !error && !programs.length ? <EmptyState icon={<FolderKanban size={21} />} title="还没有可显示的项目" description="项目创建后会按你的访问权限显示在这里。" /> : null}
