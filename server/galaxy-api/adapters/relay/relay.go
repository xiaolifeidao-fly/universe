...
func (a *Adapter) Kind() contract.KindSpec {
	spec := contract.KindSpec{
		...
		Families:  []string{FamilyAnthropic, FamilyOpenAI},
	}
	for _, family := range spec.Families {
		spec.Providers = append(spec.Providers, a.options.Providers[family])
	}
	...
	spec.Input.MaxInlineBytes = int(a.options.BodyLimit)
	return spec
}
