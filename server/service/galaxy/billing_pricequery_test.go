...
func TestPriceLookupStaysOnTheUniqueKey(t *testing.T) {
	...
	if _, err := service.priceTable(context.Background(), "llm.chat", "claude-opus-5", time.Now()); err != nil {
		t.Fatalf("取价失败：%v", err)
	}
	...
	if !strings.Contains(query, "ORDER BY kind, model_id, unit, effective_from") {
