...
// 手写 manager.sql 必然和 AutoMigrate 建出来的对不上，字段类型和索引名都会漂。
...
// TestDumpDDL 只在设置了 MANAGER_DDL_OUT 时才写文件，平时是个空跑的 no-op。
// 重新生成：MANAGER_DDL_OUT=/tmp/manager.sql go test ./service/manager/internal/repository/ -run TestDumpDDL
func TestDumpDDL(t *testing.T) {
	out := os.Getenv("MANAGER_DDL_OUT")
	if out == "" {
		t.Skip("设置 MANAGER_DDL_OUT 才导出")
	}
...
// TestGalaxySQLCoversEveryTable 盯住 manager.sql 的漂移。
//
// 它不比对列 —— 那太脆，改个字段长度就红。它只问一句：models() 里的每张表，
// manager.sql 里有没有？漏一张的后果是只跑 SQL 建库的环境在运行时报「表不存在」，
// 而这种错要等到那条代码路径第一次被走到才暴露。
func TestGalaxySQLCoversEveryTable(t *testing.T) {
	// 从 server/service/manager/internal/repository 数四级回到 server/。
	raw, err := os.ReadFile("../../../../manager.sql")
	if err != nil {
		// 不 Skip：文件不在本身就是要防的那件事，Skip 会让守卫悄悄失效。
		t.Fatalf("读不到 server/manager.sql：%v", err)
	}
	schema := string(raw)
	for _, model := range models() {
		...
		if !strings.Contains(schema, "`"+table+"`") {
			t.Errorf("manager.sql 里没有 %s —— 重新导出：见本文件顶部的 MANAGER_DDL_OUT 说明", table)
		}
	}
}
