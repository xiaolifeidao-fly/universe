...
if [[ ! -f "${PID_FILE}" ]]; then
  echo "galaxy-consumer-api is not managed by this script (no ${PID_FILE})."
  ...
  if [[ -n "${holder}" ]]; then
    ...
    echo "If that is your galaxy-consumer-api, stop it with: kill ${holder}"
  fi
  exit 0
fi
...
if kill -0 "${pid}" 2>/dev/null; then
  echo "galaxy-consumer-api did not stop cleanly (pid ${pid}); escalate with: kill -9 ${pid}"
  exit 1
fi

rm -f "${PID_FILE}"
echo "galaxy-consumer-api stopped"
