...
usage() {
  ...
  echo "Example: $0" >&2
  echo "Example: $0 galaxy-consumer-api-linux-amd64.tar.gz $SCRIPT_DIR" >&2
}
...
# 那时 start.sh 会以「找不到可执行文件」的形式失败，很难一眼看出是权限问题。
for name in bin/galaxy-consumer-api galaxy-consumer-api start.sh stop.sh; do
